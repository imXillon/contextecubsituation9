...
$confirmationreserv = [System.Windows.Forms.MessageBox]::Show($messageConfirmation, "Confirmation", [System.Windows.Forms.MessageBoxButtons]::YESNO)

if ($confirmationreserv -eq "YES") {
    
    $adresseIpReservation = [Microsoft.VisualBasic.Interaction]::InputBox("Quelle Addresse voulez vous réserver ?","")
    $adresseMac = [Microsoft.VisualBasic.Interaction]::InputBox("Quelle Adresse MAC souhaitez vous réserver a cette adresse ?","")
